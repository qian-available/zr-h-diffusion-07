...
已完成的高精度 Zr96 与 H2 保留
  ...
  -> TT_c 的CI-NEB已提交并正常进入电子迭代
  -> 后续按 TT CI完成→TO预NEB→TO CI 顺序推进
```
...
TT CI-NEB已提交，当前输出显示VASP完成输入检查并进入DAV电子迭代；是否收敛及最终势垒
须等作业结束后再由检查脚本确认。
...
均已生成；当前按TT CI完成后再推进TO的顺序执行。在TT CI完成前没有可报告的最终NEB势垒。
