...
超算原始结果已从校验通过的下载归档同步到本目录的四个计算子目录；GitHub 展示不依赖仓库
外部路径。
...
- [`initial_metrics.tsv`](analysis/initial_metrics.tsv)：任务级能量、力、步数、耗时和结构指标；
- [`relax_trajectory.tsv`](analysis/relax_trajectory.tsv)：T/O 逐离子步能量、力和 H 位移；
- [`neighbor_distances.tsv`](analysis/neighbor_distances.tsv)：T/O 排序 H-Zr 距离。
...
![T/O relaxation convergence](analysis/01_relax_convergence.png)
...
![T/O local coordination](analysis/03_local_coordination.png)

图2：T/O 末态第一配位壳层示意。图中距离由周期性最小镜像计算。

![T initial and final H positions](analysis/03b_t_h_initial_final.png)
...
![Sorted H-Zr distances](analysis/04_neighbor_distances.png)
...
![Hydrogen solution energies](analysis/02_solution_energies.png)
...
最终NEB势垒只在相应CI阶段通过收敛检查后报告。O离位扰动/声子复核可独立追加，但不作为
这两条玩具路径的前置任务。
...
使用含 NumPy 和 Pillow 的 Python 环境，从仓库根目录执行：
